...
export const StepperStep = ({ index, active, name }: StepperStepProps) => {
  return (
    <div className="relative flex flex-col items-center">
      <div
        className={cn(
          "flex h-[24px] w-[24px] items-center justify-center rounded-lg bg-primary",
          active ? "" : "opacity-40"
        )}
      >
      ...
      <p
        className={cn(
          "absolute translate-y-[32px] whitespace-nowrap text-sm font-bold text-primary",
          active ? "" : "opacity-40"
        )}
      >
        {name}
...
const StepperSeparator = ({ active }: { active: boolean }) => {
  return (
    <div
      className={cn(
        "h-0.5 flex-1 translate-y-[12px] bg-primary",
        active ? "" : "opacity-40 "
      )}
    />
  )
